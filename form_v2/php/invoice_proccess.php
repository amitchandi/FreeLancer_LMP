...
$database->query('INSERT INTO Purchases(CustomerID, PDate, Paid) VALUES(:cust_id, :pdate, :paid)');
$database->bind(':cust_id', $cust_id, PDO::PARAM_INT);
$database->bind(':pdate', $date, PDO::PARAM_STR);
$database->bind(':paid', $paid, PDO::PARAM_BOOL);
$database->execute();
...
// get Invoice_Number of inserted row
$database->query('SELECT Invoice_Number from Purchases WHERE ID = :pur_id');
$database->bind(':pur_id', $pur_id, PDO::PARAM_INT);
$Invoice_Number = $database->resultset()[0]['Invoice_Number'];
...
<p style="text-align: right;"><em><span style="text-decoration: underline;"><b>INVOICE NO. :</b></span></em> {$Invoice_Number}</p>
